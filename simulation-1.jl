...
# w_hedge = 1/N * [ones(N0);N-N0]
# w_hedge0 = 1/N * [ones(N0);N-N0]
# w_care =   1/N * [ones(N0);N-N0]
w_hedge = [zeros(N0);1]
w_hedge0 = [zeros(N0);1]
w_care =  [zeros(N0);1]
w_meta_hedge =  [zeros(1);1]
w_metacare =  [zeros(N0);1]
...
for t in 2:T
  ...
  # losses = [t%2;(t+1)%2;1]
  # rand1 = rand(Bernoulli(0.01),N0)
  rand1 = [ones(N0÷2)*iseven(t); ones(N0-N0÷2)*isodd(t)]

  losses = [rand1;1]  # rand0 = rand(Bernoulli(0.1))
  ...
  global norm_care[t] = sum(abs.(w_care .- 1/N0 * [ones(N0);0]))
  global norm_hedge[t] = sum(abs.(w_hedge .- 1/N0 * [ones(N0);0]))
  global norm_hedge0[t] = sum(abs.(w_hedge0 .- 1/N0 * [ones(N0);0]))
  global norm_metacare[t] = sum(abs.(w_metacare .- 1/N0 * [ones(N0);0]))
...
p2 = plot(1:t,Regret_hedge[1:t], label="Hedge", ylabel = "Regret", xlabel="Time",legend=:topleft,yaxis=:log, xaxis=:log, linecolor = :blue)
# plot!(1:t,Regret_hedge0[1:t],label = "Hedge 1", linecolor = :red)
plot!(1:t,Regret_care[1:t],label = "FTRL-CARE", linecolor = :green)
plot!(1:t,Regret_metacare[1:t],label = "Meta-CARE", linecolor = :purple)
# plot!(1:t,1:t,label = "T", linestyle = :dot)
# plot!(1:t, sqrt.((1+log(N0))*(1:t)),label = "sqrt(T (1+logN0))", linestyle = :dot)
# plot!(1:t, (1+log(N0))*sqrt.(1:t),label = "sqrt(T) (1+logN0) ", linestyle = :dot)    
